...
def move(grid: list[list[str]], pos: tuple[int,int], dir: tuple[int,int]) -> tuple[list[list[str]],tuple[int,int]]: # returns grid and object new position
    ...
    if(new_obj == "#"):
    ...
    elif(new_obj == "[") or (new_obj == "]"):
        if((curr_obj == "[") and (dx == 1)) or ((curr_obj == "]") and (dx == -1)):
            (grid,(temp_x,temp_y)) = move(copy.deepcopy(grid),(x+dx,y),dir)
            # print()            
        else:
            if(new_obj == "["):
                (temp_grid,(temp_x,temp_y)) = move(copy.deepcopy(grid),(new_x,new_y),dir)
                # print()
                if(dy != 0):
                    (temp_grid,(temp1_x,temp1_y)) = move(copy.deepcopy(temp_grid),(new_x+1,new_y),dir)
                    # print()
                if((temp_x == new_x) and (temp_y == new_y)) or ((dy != 0) and (temp1_x == new_x + 1) and (temp1_y == new_y)):
                    return (grid,(x,y))
                else:
                    temp_grid[new_y][new_x] = curr_obj
                    temp_grid[y][x] = "."
                    return (temp_grid,(new_x,new_y))
        ...
        if(temp_x == new_x) and (temp_y == new_y):
            ...
            if(curr_obj == "[") and (dx == 1):                
                grid[new_y][new_x] = curr_obj
                grid[new_y][new_x+1] = "]"
                grid[y][x] = "."
            elif(curr_obj == "]") and (dx == -1):
                grid[new_y][new_x] = curr_obj
                grid[new_y][new_x-1] = "["
                grid[y][x] = "."
            return (grid,(new_x,new_y))
