...
def dijkstra(grid : list[list[str]], start_byte : tuple[int]):
...
if __name__ == "__main__":
    ...
    grid = create_memory_grid(grid,bad_memory,byte_count)
    ...
    # visited_reverse = dijkstra(grid, (x_limits-1,y_limits-1))
    
    # for y in range(y_limits):
    #     for x in range(x_limits):
    #         if((x,y) not in visited_reverse) or ((x,y) not in visited):
    #             continue
    #         if(visited_reverse[(x,y)] + visited[(x,y)] == least_steps):
    #             grid[y][x] = "O"
